...
if [[ -n $1 ]]; then
    # 정규표현식을 이용하여 IP 범위를 정상적으로 입력했는지 확인
    range_chk=$(echo "$1" | grep -E "^[0-9]{1,3}.[0-9]{1,3}.[0-9]{1,3}.0/[0-9]{2}$" | wc -l)
    
    # 정규 표현식과 드르다면 메시지를 출력하고 스크립트를 종료
...
# chrony가 설치되어 있는지, ntp가 설치되어 있는지 환경 설정파일을 통해 확인
